...
- 冲突提示：同时段比赛会用 `⚠` 标记。
- 赛前提醒：默认赛前 30 分钟提醒，Windows Toast 成功则系统通知，失败时回退为弹窗。
...
程序只使用 Python 标准库，不需要安装第三方依赖。Windows 10/11 上 Toast 通知通过 PowerShell 调用 WinRT；如果系统限制导致调用失败，会自动回退到经典 `messagebox` 弹窗。
